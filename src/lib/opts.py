...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--test', default=False, action='store_true')
    ...
    self.parser.add_argument('--demo', default='../data/test/test1.jpg', 
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    # default => input_h = 1, input_w = -1 
    # default_resolution = [512, 512]
    # 논문에서의 train input resolution = [512, 512]
    opt.mean, opt.std = dataset.mean, dataset.std
    # mean : [0.408, 0.447, 0.470]
    # std : [0.289, 0.271, 0.278]
    ...
    if opt.task == 'ctdet': # ctdet : object detection
      # assert opt.dataset in ['pascal', 'coco']
      opt.heads = {'hm': opt.num_classes,
                    # hm : heatmap
                    # wh : bounding box size
                   'wh': 2 if not opt.cat_spec_wh else 2 * opt.num_classes} # else일 경우 160
                   # cat_spec_wh = category specific bounding box size
      if opt.reg_offset:
            # opt.reg_offset : regress local offset
            # reg : regression
