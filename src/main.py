...
detf main(opt):
    ...
    # opt.arch = dla_34
    # opt.heads = heatmap : 80 / wh(bounding box size) : 2
    # opt.head_conv = -1
    ...
    # opt.chunk_sizes = master batch size : -1 / chunck_size : 1 -> master batch size의 절대값
    ...
    for epoch in range(start_epoch + 1, opt.num_epochs + 1):
        ...
        if opt.val_intervals > 0 and epoch % opt.val_intervals == 0: # 약 28번
            # opt.val_intervals => default = 5 validation이 실행할 epoch 수
